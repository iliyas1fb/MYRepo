...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			{
				if (xhr.status == 901) {
					var url123 = "${contextPath}/app/auth/sessionTimeOut";
		...
		isSafari = ($.browser.webkit || $.browser.safari) && (window.navigator.userAgent.toLowerCase().indexOf('chrome') < 0 ||
...
$.jgrid.extend({
	...
	showHideCol : function(colname,show) {
		return this.each(function() {
			var $t = this, fndh=false, brd=($.browser.webkit || $.browser.safari) && (window.navigator.userAgent.toLowerCase().indexOf('chrome') < 0 ||
			        parseFloat($.browser.version)<536.5)? 0: $t.p.cellLayout, cw;
			if (!$t.grid ) {return;}
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			if (!this.grid ) {return;}
			var $t = this, cw,
			initwidth = 0, brd=($.browser.webkit || $.browser.safari) && (window.navigator.userAgent.toLowerCase().indexOf('chrome') < 0 ||
			        parseFloat($.browser.version)<536.5)? 0: $t.p.cellLayout, lvc, vc=0, hs=false, scw=$t.p.scrollOffset, aw, gw=0,
			cl = 0,cr;
